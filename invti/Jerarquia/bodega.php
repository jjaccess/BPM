...
function miFuncion() {
	$('#tablaCategoriaLoad').load("TablaBod.php");
		}
  ...
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Asignar nueva bodega</h5>
		...
		<input type="text" hidden="" id="login" name="login" value="<?php echo $rowuser['usuario'] ?>">
			...
			<select  class="form-control input-sm" name="usuario" id="usuario">
                            ...
                            <?php
															...
															WHERE id_tipo in (1,3,15)
															AND usuario not in (SELECT responsable FROM bodegas)
															AND activacion = 1
															ORDER BY nombre");
...
<div class="modal fade" id="actualizaCategoria" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog  modal-sm" role="document">
    <div class="modal-content">
	  ...
	  <label>Bodega:</label>
	  <input type="text" class="form-control input-sm" id="categoriaU" readonly="" name="categoriaU">
		...
		$(document).ready(function(){
			$('#btnActualizaCategoria').click(function(){
				...
				$.ajax({
					type:"POST",
					data:datos,
					url:"procesos/actualizaBodega.php",
					success:function(r){
						console.log(r);
						if(r==1){
							$('#tablaCategoriaLoad').load("TablaBod.php");
	...
	<script type="text/javascript">
		function agregaDato(idCategoria,categoria,responsable,login){
		...
		function eliminaCategoria(idcategoria){
			alertify.confirm('¿Desea eliminar esta bodega?', function(){ 
				$.ajax({
					...
					url:"procesos/eliminarBodega.php",
					success:function(r){
						console.log(r);
						if(r==1){
							$('#tablaCategoriaLoad').load("TablaBod.php");
