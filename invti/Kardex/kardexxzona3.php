...
<div class="container">
	<div class="row">
        <div class="col-sm-12">
<form class="form-inline formulariolinea" action="kardexxzona3.php" method="get" autocomplete="off" style="text-align: center;">
		...
		<h2 class="text-primary">
			...
			<select class="form-control" name="busqueda" id="busqueda" onChange="obtenerTipos(this.value);">
        		<option value="<?php if(isset($clasificacion)) echo $clasificacion; ?>" required>Seleccione Bodega:</option>
						<?php
						
							while($row= $consulta_bodega->fetch_object())
							{
			...
			<select class="form-control" name="clasificacion" onChange="obtenerTipos(this.value);">
	        	<option value="<?php if(isset($clasificacion)) echo $clasificacion; ?>" required>Seleccione Clasificacion:</option>
							<?php
 ...
 <?php  
	...
	//Paginador
			$sql_registe = mysqli_query($mysqli,"
			
SELECT COUNT(*) as total_registro from
(select '1- Bodega' ubc,
...
FROM bodegas,invti_bodegas, invti_articulos, clasificaciones, invti_marca, invti_tipoart
WHERE invti_bodegas.BODEGA = '" . $busqueda . "'
AND invti_bodegas.clasificacion = '" . $clasificacion . "'
...
GROUP BY articulo
ORDER BY clasificacion, tipo, articulo)
as total_registro");
