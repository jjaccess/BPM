...
function miFuncion() {
	$('#tablaCategoriaLoad').load("TablaClase.php");
		}
  ...
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Agregar nueva clase</h5>
      ...
      <div class="modal-body">
        <form id="frmCategorias">                           
            <label>Seleccione artículo:</label>
			<select  class="form-control input-sm" name="id_tipoart" id="id_tipoart">
                            <option value="" required >Selección:</option>
							<?php
															
                                                            $query1 = $mysqli -> query ("SELECT id, articulo FROM invti_articulos 
															WHERE id not in (SELECT id_art FROM invti_clasearticulos)
															ORDER BY articulo;");
                                                                                              
                                                            while ($tipo = mysqli_fetch_array($query1)) {
                                                                                                  
                                                              echo '<option value="'.$tipo['id'].'">'.$tipo['articulo'].'</option>';
                                                                                                  
                                                            }
                                                          ?>
                        </select>
						<label>Seleccione Clase:</label>
			<select  class="form-control input-sm" name="categoria" id="categoria">
                            <option value="" required >Selección:</option>
                            <?php
															
                                                            $query1 = $mysqli -> query ("SELECT clase FROM clases GROUP BY clase  ORDER BY clase");
                                                                                              
                                                            while ($tipo = mysqli_fetch_array($query1)) {
                                                                                                  
                                                              echo '<option value="'.$tipo['clase'].'">'.$tipo['clase'].'</option>';
  ...
  <div class="modal-dialog modal-sm" role="document">
		...
		$(document).ready(function(){
			...
			$('#btnAgregaCategoria').click(function(){
				...
				$.ajax({
					...
					$('#tablaCategoriaLoad').load("TablaClase.php");
					alertify.success("Clase agregada con exito :D");
				}else{
					alertify.error("No se pudo agregar Clase");
				}
			}
		});
			});
		...
		$(document).ready(function(){
			$('#btnActualizaCategoria').click(function(){
				...
				$.ajax({
					type:"POST",
					data:datos,
					url:"procesos/actualizaClase.php",
					success:function(r){
						if(r==1){
							$('#tablaCategoriaLoad').load("TablaClase.php");
	...
	<script type="text/javascript">
		...
		function eliminaCategoria(idcategoria){
			alertify.confirm('¿Desea eliminar esta Clase?', function(){ 
				$.ajax({
					type:"POST",
					data:"idcategoria=" + idcategoria,
					url:"procesos/eliminarClase.php",
					success:function(r){
						if(r==1){
							$('#tablaCategoriaLoad').load("TablaClase.php");
